...
internal class ApproveConnectConfirmViewModel(
    ...
    val headerViewItemList: LiveData<List<ViewItemCloneable>> = combineSources(requestDetectState) {
        ...
        list.add(state.data.toHeaderViewItem())
    ...
    val infoViewItemList: LiveData<List<ViewItemCloneable>> = combineSources(requestDetectState) {
        ...
        list.addAll(state.data.getInfo())
...
//        }
    ...
    @VisibleForTesting
    val dataInfoViewItemList: LiveData<List<ViewItemCloneable>> = combineSources(headerViewItemList, infoViewItemList, messageViewItemList, bottomViewItemList) {

        val list = arrayListOf<ViewItemCloneable>()

        list.addAll(headerViewItemList.getOrEmpty())

        list.addAll(infoViewItemList.getOrEmpty())

        messageViewItemList.getOrEmpty().takeIf { it.isNotEmpty() }?.let {
...
//
//            approveConnectState.postValue(ResultState.Failed("", AppExceptionV2(code = ErrorCode.PLEASE_CONFIRM)))
//
//            return@launch
//        }
    ...
    private fun Request.getInfo(): List<ViewItemCloneable> {
        ...
        listOf(
            TextImage(R.drawable.img_tick_accent_24dp, 12.toPx()),
        ...
        listOf(
            TextImage(R.drawable.img_close_on_background_variant_24dp, 12.toPx()),
